...
struct PlayerSlotT {
	...
	std::array<LPITEM,SWITCHBOT_SLOT_MAX> pSBItems;
#endif
#ifdef ENABLE_ACCE_COSTUME_SYSTEM
...
class CHARACTER : public CEntity, public CFSM, public CHorseRider
{
	...
	public:
		...
		void			UpdateNewTitle();
		const char *	GetNewTitle();
		void			SetNewTitle(const char* newTitle);
		void			SetNewTitleColor(int index, BYTE val);
		void			SetNewTitleColors(const BYTE* customcolors);
		BYTE*			GetNewTitleColors() const;
		BYTE			GetNewTitleColor(int index);

		void			UpdateNewAlign();
		const char *	GetNewAlign();
		void			SetNewAlign(const char* newAlign);
		void			SetNewAlignColor(int index, BYTE val);
		void			SetNewAlignColors(const BYTE* colors);
		BYTE*			GetNewAlignColors() const;
		BYTE			GetNewAlignColor(int index);
#endif
...
//		bool			IsPet() const		{ return m_bCharType == CHAR_TYPE_PET; }
	...
	protected:
		...
		BYTE			m_bCharType;
#ifdef ENABLE_NEW_TITLE_SYSTEM
		std::string		m_stNewTitle; 
		BYTE			m_bNewTitleColor[NEW_COLOR_MAX_NUM];

		std::string		m_stNewAlign; 
		BYTE			m_bNewAlignColor[NEW_COLOR_MAX_NUM];
#endif
