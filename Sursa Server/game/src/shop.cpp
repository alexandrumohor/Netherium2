...
int CShop::Buy(LPCHARACTER ch, BYTE pos)
{
...
#ifdef ENABLE_SHOP_USE_CHEQUE
	if ((unsigned long long)dwPrice > 0 && (int)dwCheque > 0) // Yang-Cheque
	{
		if (ch->GetGold() < (unsigned long long)dwPrice || ch->GetCheque() < (int)dwCheque)
			return SHOP_SUBHEADER_GC_NOT_ENOUGH_MONEY_CHEQUE;
	}
	else if ((unsigned long long)dwPrice > 0 && (int)dwCheque <= 0) // Yang
	{

#ifdef ENABLE_REMOVE_LIMTI_GOLD
	...
	else if ((unsigned long long)dwPrice <= 0 && (int)dwCheque > 0) // cheque
	{
