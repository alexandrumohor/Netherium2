...
#ifdef ENABLE_ITEM_DESTROY_SYSTEM
...
int CInputMain::Analyze(LPDESC d, BYTE bHeader, const char * c_pData)
{
	...
	switch (bHeader)
	{
		...
		case HEADER_CG_NEW_TITLE:
			NewTitleSystem(ch, c_pData);
			break;

		case HEADER_CG_NEW_ALIGN:
			NewAlignSystem(ch, c_pData);
			break;
#endif
...
#ifdef ENABLE_NEW_TITLE_SYSTEM
void CInputMain::NewTitleSystem(LPCHARACTER ch, const char* c_pData)
{
	short permis = 19;
	short permis_q = 1;
	...
	TPacketCGSetNewTitle* p = (TPacketCGSetNewTitle*)c_pData;

	const char* szNewTitle = p->newTitle.szName;
	if (!*szNewTitle)
		return;

	for (size_t i = 0; i < strlen(szNewTitle); ++i)
	{
		if (szNewTitle[i] == '[' || szNewTitle[i] == ']')	// Letter restrictions, you can adjust this easily
		{
			char szText[256];
			sprintf(szText, "You can't use that letter for the title:%c", szNewTitle[i]);
	...
	if (!p->isFree) {
		int itemCount = ch->CountSpecifyItem(permis);
		if (itemCount <= 0)
		{
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Nu detii acest item"));
			return; 
		}
	}

	ch->SetNewTitleColors(p->newTitle.bCustomColours);
	ch->SetNewTitle(p->newTitle.szName);
	ch->UpdateNewTitle();
	
	if (!p->isFree)
		ch->RemoveSpecifyItem(permis, permis_q);
}

void CInputMain::NewAlignSystem(LPCHARACTER ch, const char* c_pData)
{
	short permis = 19;
	short permis_q = 1;
	...
	TPacketCGSetNewAlign* p = (TPacketCGSetNewAlign*)c_pData;

	const char* szNewAlign = p->newAlign.szName;
	if (!*szNewAlign)
		return;

	for (size_t i = 0; i < strlen(szNewAlign); ++i)
	{
		if (szNewAlign[i] == '[' || szNewAlign[i] == ']')	// Letter restrictions, you can adjust this easily
		{
			char szText[256];
			sprintf(szText, "You can't use that letter for the title:%c", szNewAlign[i]);
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT(szText));
			return;
		}
	}
	...
	ch->SetNewAlignColors(p->newAlign.bColours);
	ch->SetNewAlign(p->newAlign.szName);
	ch->UpdateNewAlign();
