...
void CHARACTER::Initialize()
{
	...
	m_stNewTitle = "";
	memset(m_bNewTitleColor, 0, sizeof(m_bNewTitleColor));

	m_stNewAlign = "";
	memset(m_bNewAlignColor, 0, sizeof(m_bNewAlignColor));
#endif
}
...
void CHARACTER::OpenMyShop(const char * c_pszSign, TShopItemTable * pTable, BYTE bItemCount)
{
	...
	{
		sys_err("[OVERFLOW_GOLD] Overflow (GOLD_MAX_MAX) id %u name %s", GetPlayerID(), GetName());
...
#endif
#ifdef ENABLE_NEW_TITLE_SYSTEM
			strlcpy(addPacket.newTitle.szName, GetNewTitle(), sizeof(addPacket.newTitle.szName));
			memcpy(addPacket.newTitle.bCustomColours, GetNewTitleColors(), sizeof(addPacket.newTitle.bCustomColours));

			strlcpy(addPacket.newAlign.szName, GetNewAlign(), sizeof(addPacket.newAlign.szName));
			memcpy(addPacket.newAlign.bColours, GetNewAlignColors(), sizeof(addPacket.newAlign.bColours));
#endif
#ifdef __SKILL_COLOR_SYSTEM__
...
#ifdef ENABLE_NEW_TITLE_SYSTEM
	strlcpy(tab.newTitle.szName, GetNewTitle(), sizeof(tab.newTitle.szName));
	memcpy(tab.newTitle.bCustomColours, GetNewTitleColors(), sizeof(tab.newTitle.bCustomColours));

	strlcpy(tab.newAlign.szName, GetNewAlign(), sizeof(tab.newAlign.szName));
	memcpy(tab.newAlign.bColours, GetNewAlignColors(), sizeof(tab.newAlign.bColours));
#endif
}
...
#ifdef ENABLE_NEW_TITLE_SYSTEM
	m_stNewTitle = t->newTitle.szName;
	memcpy(m_bNewTitleColor, t->newTitle.bCustomColours, sizeof(m_bNewTitleColor));

	m_stNewAlign = t->newAlign.szName;
	memcpy(m_bNewAlignColor, t->newAlign.bColours, sizeof(m_bNewAlignColor));
#endif
...
bool CHARACTER::WonExchange(const std::string & option, int value)
{
	...
	if (option == "sell")
	{
		...
		if (GetGold() + totalYang >= GOLD_MAX_MAX)
		{
...
#ifdef ENABLE_NEW_TITLE_SYSTEM
const char * CHARACTER::GetNewTitle()
{
	return this->m_stNewTitle.c_str();
}

void CHARACTER::SetNewTitle(const char * newTitle)
{
	m_stNewTitle = newTitle;
}
BYTE* CHARACTER::GetNewTitleColors() const
{
	return const_cast<BYTE*>(m_bNewTitleColor);
}

BYTE CHARACTER::GetNewTitleColor(int index)
{
	assert(index < NEW_COLOR_MAX_NUM);
	return m_bNewTitleColor[index];
}
void CHARACTER::SetNewTitleColor(int index, BYTE val)
{
	assert(index < NEW_COLOR_MAX_NUM);
	m_bNewTitleColor[index] = val;
}

void CHARACTER::SetNewTitleColors(const BYTE* customcolors)
{
	if (!customcolors)
		return;

	memcpy(m_bNewTitleColor, customcolors, sizeof(m_bNewTitleColor));
}
void CHARACTER::UpdateNewTitle()
{
	TPacketGCUpdateNewTitle pack;
	pack.header = HEADER_GC_UPDATE_NEW_TITLE;
	pack.dwVID = GetVID();
	strlcpy(pack.newTitle.szName, GetNewTitle(), sizeof(pack.newTitle.szName));
	memcpy(pack.newTitle.bCustomColours, m_bNewTitleColor, sizeof(pack.newTitle.bCustomColours));
	PacketAround(&pack, sizeof(pack));
}


const char * CHARACTER::GetNewAlign()
{
	return this->m_stNewAlign.c_str();
}

void CHARACTER::SetNewAlign(const char * newAlign)
{
	m_stNewAlign = newAlign;
}
BYTE* CHARACTER::GetNewAlignColors() const
{
	return const_cast<BYTE*>(m_bNewAlignColor);
}

BYTE CHARACTER::GetNewAlignColor(int index)
{
	assert(index < NEW_COLOR_MAX_NUM);
	return m_bNewAlignColor[index];
}
void CHARACTER::SetNewAlignColor(int index, BYTE val)
{
	assert(index < NEW_COLOR_MAX_NUM);
	m_bNewAlignColor[index] = val;
}

void CHARACTER::SetNewAlignColors(const BYTE* colors)
{
	if (!colors)
		return;

	memcpy(m_bNewAlignColor, colors, sizeof(m_bNewAlignColor));
}
void CHARACTER::UpdateNewAlign()
{
	TPacketGCUpdateNewAlign pack;
	pack.header = HEADER_GC_UPDATE_NEW_ALIGN;
	pack.dwVID = GetVID();
	strlcpy(pack.newAlign.szName, GetNewAlign(), sizeof(pack.newAlign.szName));
	memcpy(pack.newAlign.bColours, m_bNewAlignColor, sizeof(pack.newAlign.bColours));
	PacketAround(&pack, sizeof(pack));
}
#endif