...
void CHARACTER::Dead(LPCHARACTER pkKiller, bool bImmediateDead)
{
	...
	if (IsMonster() || IsStone())
	{
		...
		const DWORD STRUCT_MAX_NUM = 8096;
		
		static SLevelingMaps structInfo[STRUCT_MAX_NUM] =
		{
			{1, 5, 1, 5, 20000, 40000},
			{5, 10, 1, 10, 20000, 40000},
			{10, 15, 1, 12, 20000, 40000},
			{15, 20, 1, 15, 20000, 40000},
			{20, 30, 1, 17, 20000, 40000},
			{1, 5, 21, 5, 20000, 40000},
			{5, 10, 21, 10, 20000, 40000},
			{10, 15, 21, 12, 20000, 40000},
			{15, 20, 21, 15, 20000, 40000},
			{20, 30, 21, 17, 20000, 40000},
			{1, 5, 41, 5, 20000, 40000},
			{5, 10, 41, 10, 20000, 40000},
			{10, 15, 41, 12, 20000, 40000},
			{15, 20, 41, 15, 20000, 40000},
			{20, 30, 41, 17, 20000, 40000},
		};
		
		for (int i=0; i<STRUCT_MAX_NUM; i++)
		{			
			if (pkKiller && pkKiller->GetMapIndex() == structInfo[i].dwMapIndex)
			{
				if (pkKiller && pkKiller->GetLevel() >= structInfo[i].dwCharMinLevel && pkKiller->GetLevel() < structInfo[i].dwCharMaxLevel)
				{
					// if (pkKiller->IsPremiumPlayer())
					// {
						// pkKiller->PointChange(POINT_EXP, (pkKiller->GetNextExp() / structInfo[i].dwDivisor) * 2);
						// pkKiller->ChangeGold((number(structInfo[i].dwGoldMin, structInfo[i].dwGoldMax)) * 3);
					// }
					// else
					// {
					pkKiller->PointChange(POINT_EXP, pkKiller->GetNextExp() / structInfo[i].dwDivisor);
					pkKiller->ChangeGold(number(structInfo[i].dwGoldMin, structInfo[i].dwGoldMax));
					// }
				}
			}
		}
