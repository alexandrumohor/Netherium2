...
size_t CreatePlayerSaveQuery(char * pszQuery, size_t querySize, TPlayerTable * pkTab)
{
	...
	CDBManager::instance().EscapeString(text, pkTab->newTitle.szName, strlen(pkTab->newTitle.szName));
	queryLen += snprintf(pszQuery + queryLen, querySize - queryLen, "title = '%s', ", text);

	for (int i = 0; i < NEW_COLOR_MAX_NUM; ++i)
		queryLen += snprintf(pszQuery + queryLen, querySize - queryLen, "title_color%d = '%u', ", i, pkTab->newTitle.bCustomColours[i]);

	CDBManager::instance().EscapeString(text, pkTab->newAlign.szName, strlen(pkTab->newAlign.szName));
	queryLen += snprintf(pszQuery + queryLen, querySize - queryLen, "align = '%s', ", text);

	for (int i = 0; i < NEW_COLOR_MAX_NUM; ++i)
		queryLen += snprintf(pszQuery + queryLen, querySize - queryLen, "align_color%d = '%u', ", i, pkTab->newAlign.bColours[i]);
#endif
...
#endif
					"FROM item%s WHERE owner_id=%d AND (`window` in ('INVENTORY','EQUIPMENT','DRAGON_SOUL_INVENTORY','SWITCHBOT,'BELT_INVENTORY'))",
					GetTablePostfix(), pTab->id);
...
#ifdef ENABLE_NEW_TITLE_SYSTEM
				", title, title_color0, title_color1, title_color2, title_color3, align, align_color0, align_color1, align_color2, align_color3 "
#endif
#ifdef ENABLE_CHEQUE_SYSTEM
				", cheque "
#endif
...
#ifdef ENABLE_NEW_TITLE_SYSTEM
	if (row[col])
		strcpy(pkTab->newTitle.szName, row[col]);
	else
		memset(pkTab->newTitle.szName, '\0', sizeof(pkTab->newTitle.szName));

	col++;
	
	for (int i = 0; i < NEW_COLOR_MAX_NUM; ++i)
	{
		str_to_number(pkTab->newTitle.bCustomColours[i], row[col]);
		col++;
	}

	if (row[col])
		strcpy(pkTab->newAlign.szName, row[col]);
	else
		memset(pkTab->newAlign.szName, '\0', sizeof(pkTab->newAlign.szName));

	col++;
	
	for (int i = 0; i < NEW_COLOR_MAX_NUM; ++i)
	{
		str_to_number(pkTab->newAlign.bColours[i], row[col]);
		col++;
	}
#endif
#ifdef ENABLE_CHEQUE_SYSTEM
...
void CClientManager::__RESULT_PLAYER_DELETE(CPeer *peer, SQLMsg* msg)
{
	...
	if (msg->Get() && msg->Get()->uiNumRows)
	{
		...
		snprintf(queryStr, sizeof(queryStr), "DELETE FROM item%s WHERE owner_id=%d AND (`window` in ('INVENTORY','EQUIPMENT','DRAGON_SOUL_INVENTORY','SWITCHBOT,'BELT_INVENTORY'))", GetTablePostfix(), pi->player_id);
		CDBManager::instance().DirectQuery(queryStr);
