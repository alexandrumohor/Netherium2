...
void CPythonNetworkStream::GamePhase()
{
    ...
    while (ret)
	{
		...
		switch (header)
		{
			...
			case HEADER_GC_UPDATE_NEW_TITLE:
				ret = RecvCharacterUpdateNewTitlePacket();
				break;

			case HEADER_GC_UPDATE_NEW_ALIGN:
				ret = RecvCharacterUpdateNewAlignPacket();
				break;
#endif
...
#ifdef ENABLE_NEW_TITLE_SYSTEM
bool CPythonNetworkStream::SendNewTitlePacket(bool isFree, const char* newTitle, BYTE r, BYTE g, BYTE b, BYTE a)
{
	TPacketCGSetNewTitle pack;
	pack.header = HEADER_CG_NEW_TITLE;
	pack.isFree = isFree;
	strcpy(pack.newTitle.szName, newTitle);
	pack.newTitle.bCustomColours[0] = r;
	pack.newTitle.bCustomColours[1] = g;
	pack.newTitle.bCustomColours[2] = b;
	pack.newTitle.bCustomColours[3] = a;

	if (!Send(sizeof(pack), &pack))
	{
		Tracen("CPythonNetworkStream::SendCustomTitlePacket - ERROR");
		return false;
	}
	return true;
}

bool CPythonNetworkStream::SendRemoveNewTitlePacket()
{
	TPacketCGSetNewTitle pack;
	pack.header = HEADER_CG_NEW_TITLE;
	pack.isFree = true;
	strcpy(pack.newTitle.szName, " ");
	pack.newTitle.bCustomColours[0] = 255;
	pack.newTitle.bCustomColours[1] = 255;
	pack.newTitle.bCustomColours[2] = 255;
	pack.newTitle.bCustomColours[3] = 255;

	if (!Send(sizeof(pack), &pack))
	{
		Tracen("CPythonNetworkStream::SendTitlePacket - ERROR");
		return false;
	}
...
bool CPythonNetworkStream::SendNewAlignPacket(bool isFree, const char* newAlign, BYTE r, BYTE g, BYTE b, BYTE a)
{
	TPacketCGSetNewAlign pack;
	pack.header = HEADER_CG_NEW_ALIGN;
	pack.isFree = isFree;
	strcpy(pack.newAlign.szName, newAlign);
	pack.newAlign.bColours[0] = r;
	pack.newAlign.bColours[1] = g;
	pack.newAlign.bColours[2] = b;
	pack.newAlign.bColours[3] = a;
...
bool CPythonNetworkStream::SendRemoveNewAlignPacket()
{
	TPacketCGSetNewAlign pack;
	pack.header = HEADER_CG_NEW_ALIGN;
	pack.isFree = true;
	strcpy(pack.newAlign.szName, " ");
	pack.newAlign.bColours[0] = 255;
	pack.newAlign.bColours[1] = 255;
	pack.newAlign.bColours[2] = 255;
	pack.newAlign.bColours[3] = 255;
