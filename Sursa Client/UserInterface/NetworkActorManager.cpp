...
#endif
#ifdef ENABLE_NEW_TITLE_SYSTEM
	m_stNewTitle = src.m_stNewTitle;
	memcpy(m_bNewTitleColor, src.m_bNewTitleColor, sizeof(m_bNewTitleColor));

	m_stNewAlign = src.m_stNewAlign;
	memcpy(m_bNewAlignColor, src.m_bNewAlignColor, sizeof(m_bNewAlignColor));
#endif
...
#ifdef ENABLE_NEW_TITLE_SYSTEM
	kCreateData.m_stNewTitle = rkNetActorData.m_stNewTitle;
	memcpy(kCreateData.m_bNewTitleColor, rkNetActorData.m_bNewTitleColor, sizeof(kCreateData.m_bNewTitleColor));

	kCreateData.m_stNewAlign = rkNetActorData.m_stNewAlign;
	memcpy(kCreateData.m_bNewAlignColor, rkNetActorData.m_bNewAlignColor, sizeof(kCreateData.m_bNewAlignColor));
#endif
...
#ifdef ENABLE_NEW_TITLE_SYSTEM
void CNetworkActorManager::UpdateActorNewTitle(DWORD dwVID, const char* newTitle, const BYTE* bCustomColor)
{
	std::map<DWORD, SNetworkActorData>::iterator f = m_kNetActorDict.find(dwVID);
	if (m_kNetActorDict.end() == f)
	{
#ifdef _DEBUG
		TraceError("CNetworkActorManager::UpdateActor(dwVID=%d) - NOT EXIST VID", dwVID);
#endif
		return;
	}
	...
	if (!pkInstFind)
	{
		TraceError("No Instance by %s %u", rkNetActorData.m_stName.c_str(), rkNetActorData.m_dwVID);
		return;
	}

	pkInstFind->UpdateNewTitle(newTitle, const_cast<BYTE*>(bCustomColor));
	memcpy(rkNetActorData.m_bNewTitleColor, bCustomColor, sizeof(rkNetActorData.m_bNewTitleColor));
	rkNetActorData.m_stNewTitle = newTitle;
}

void CNetworkActorManager::UpdateActorNewAlign(DWORD dwVID, const char* newAlign, const BYTE* bColor)
{
	std::map<DWORD, SNetworkActorData>::iterator f = m_kNetActorDict.find(dwVID);
	if (m_kNetActorDict.end() == f)
	{
	...
	pkInstFind->UpdateNewAlign(newAlign, const_cast<BYTE*>(bColor));
	memcpy(rkNetActorData.m_bNewAlignColor, bColor, sizeof(rkNetActorData.m_bNewAlignColor));
	rkNetActorData.m_stNewAlign = newAlign;
}
#endif