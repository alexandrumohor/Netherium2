...
PyObject* playerSetNewTitle(PyObject* poSelf, PyObject* poArgs)
{
	bool isFree;
	if (!PyTuple_GetBoolean(poArgs, 0, &isFree))
		return Py_BadArgument();

	char* szNewTitle;
	if (!PyTuple_GetString(poArgs, 1, &szNewTitle))
		return Py_BadArgument();
	...
	if (!PyTuple_GetInteger(poArgs, 3, &g))
		return Py_BadArgument();

	if (!PyTuple_GetInteger(poArgs, 4, &b))
		return Py_BadArgument();

	if (!PyTuple_GetInteger(poArgs, 5, &a))
		return Py_BadArgument();

	CPythonNetworkStream::Instance().SendNewTitlePacket(isFree, szNewTitle, r, g, b, a);

	return Py_BuildNone();
}

PyObject* playerRemoveNewTitle(PyObject* poSelf, PyObject* poArgs)
{
	CPythonNetworkStream::Instance().SendRemoveNewTitlePacket();
	return Py_BuildNone();
}

PyObject* playerSetNewAlign(PyObject* poSelf, PyObject* poArgs)
{
	bool isFree;
	if (!PyTuple_GetBoolean(poArgs, 0, &isFree))
		return Py_BadArgument();
	...
	char* szNewAlign;
	if (!PyTuple_GetString(poArgs, 1, &szNewAlign))
		return Py_BadArgument();
	...
	CPythonNetworkStream::Instance().SendNewAlignPacket(isFree, szNewAlign, r, g, b, a);

	return Py_BuildNone();
}

PyObject* playerRemoveNewAlign(PyObject* poSelf, PyObject* poArgs)
{
	CPythonNetworkStream::Instance().SendRemoveNewAlignPacket();
	return Py_BuildNone();
}
#endif
...
#ifdef ENABLE_NEW_TITLE_SYSTEM
		{ "SetNewTitle",				playerSetNewTitle,					METH_VARARGS },
		{ "RemoveNewTitle",				playerRemoveNewTitle,				METH_VARARGS },

		{ "SetNewAlign",				playerSetNewAlign,					METH_VARARGS },
		{ "RemoveNewAlign",				playerRemoveNewAlign,				METH_VARARGS },
#endif
