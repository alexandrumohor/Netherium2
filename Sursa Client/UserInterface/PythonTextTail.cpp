...
#ifdef ENABLE_NEW_TITLE_SYSTEM
const float c_fysTitlePosition = 12.0f;
BOOL bPKNewTitleEnable = TRUE;
...
void CPythonTextTail::ArrangeTextTail()
{
	...
	for (itor = m_CharacterTextTailList.begin(); itor != m_CharacterTextTailList.end(); ++itor)
	{
...
#ifdef ENABLE_NEW_TITLE_SYSTEM
		CGraphicTextInstance* psNewTitleSistem = pTextTail->pUserNewTitleTextInstance;//aici nu era pUserCustomTitleTextInstance ?ne
		if (psNewTitleSistem)
		{
			int iWidth, iHeight;
			psNewTitleSistem->GetTextSize(&iWidth, &iHeight);
			psNewTitleSistem->SetPosition(pTextTail->x, pTextTail->y - c_fysTitlePosition, pTextTail->z);
			psNewTitleSistem->Update();
		}
#endif
...
#ifdef ENABLE_NEW_TITLE_SYSTEM
			if (pTextTail->pUserNewTitleTextInstance && pTextTail->pUserNewTitleTextInstance->GetValueStringReference() == " ")
			{
		...
		if (pTitle)
		{
			CGraphicTextInstance* pUserNewAlign = pTextTail->pUserNewAlignTextInstance;

			if (pUserNewAlign && pUserNewAlign->GetValueStringReference() != " ")
			{
				int iUTitleWidth, iUTitleHeight;
				pUserNewAlign->GetTextSize(&iUTitleWidth, &iUTitleHeight);
				pUserNewAlign->SetPosition(pTextTail->x - (iNameWidth / 2) - 3, pTextTail->y, pTextTail->z);
				pUserNewAlign->Update();
				...
				if (pLevel)
				{
					...
					pLevel->SetPosition(pTextTail->x - (iNameWidth / 2) - iUTitleWidth - 6, pTextTail->y, pTextTail->z);
			...
			else
			{
				...
				if (pLevel)
				{
					int iLevelWidth, iLevelHeight;
					pLevel->GetTextSize(&iLevelWidth, &iLevelHeight);
					pLevel->SetPosition(pTextTail->x - (iNameWidth / 2) - 3, pTextTail->y, pTextTail->z);
...
#ifdef ENABLE_NEW_TITLE_SYSTEM
		if (pTextTail->pUserNewTitleTextInstance)
		{
			pTextTail->pUserNewTitleTextInstance->Render();
		}

		if (pTextTail->pUserNewAlignTextInstance)
		{
			pTextTail->pUserNewAlignTextInstance->Render();
		}
#endif
	}
...
CPythonTextTail::TTextTail * CPythonTextTail::RegisterTextTail(DWORD dwVirtualID, const char * c_szText, CGraphicObjectInstance * pOwner, float fHeight, const D3DXCOLOR & c_rColor)
{
	...
	pTextTail->pLevelTextInstance = NULL;
#ifdef ENABLE_NEW_TITLE_SYSTEM
	pTextTail->pUserNewTitleTextInstance = NULL;
	pTextTail->pUserNewAlignTextInstance = NULL;
#endif
	return pTextTail;
}
...
void CPythonTextTail::DeleteTextTail(TTextTail * pTextTail)
{
	...
	if (pTextTail->pUserNewTitleTextInstance)
	{
		CGraphicTextInstance::Delete(pTextTail->pUserNewTitleTextInstance);
		pTextTail->pUserNewTitleTextInstance = NULL;
	}

	if (pTextTail->pUserNewAlignTextInstance)
	{
		CGraphicTextInstance::Delete(pTextTail->pUserNewAlignTextInstance);
		pTextTail->pUserNewAlignTextInstance = NULL;
	}
#endif
...
#ifdef ENABLE_NEW_TITLE_SYSTEM
void CPythonTextTail::AttachUserNewTitle(DWORD dwVID, const char* c_szText, const D3DXCOLOR& c_rColor)
{
	TTextTailMap::iterator itor = m_CharacterTextTailMap.find(dwVID);
	if (m_CharacterTextTailMap.end() == itor)
		return;
	...
	CGraphicTextInstance*& prUserNewTitle = pTextTail->pUserNewTitleTextInstance;
	if (!prUserNewTitle)
	{
		prUserNewTitle = CGraphicTextInstance::New();
		prUserNewTitle->SetTextPointer(ms_pFont);
		prUserNewTitle->SetOutline(true);

		prUserNewTitle->SetHorizonalAlign(CGraphicTextInstance::HORIZONTAL_ALIGN_CENTER);
		prUserNewTitle->SetVerticalAlign(CGraphicTextInstance::VERTICAL_ALIGN_BOTTOM);
	}

	prUserNewTitle->SetValue(c_szText);
	prUserNewTitle->SetColor(c_rColor.r, c_rColor.g, c_rColor.b);
	//'	TraceError("r : %d  -- g %d  -- b %d",c_rColor.r, c_rColor.g, c_rColor.b);
	prUserNewTitle->Update();
}

void CPythonTextTail::DetachUserNewTitle(DWORD dwVID)
{
	if (!bPKNewTitleEnable)
		return;
...
void CPythonTextTail::UpdateNewTitleColor(DWORD dwVID, const D3DXCOLOR& c_Color)
{
	TTextTailMap::iterator itor = m_CharacterTextTailMap.find(dwVID);
	if (m_CharacterTextTailMap.end() == itor)
		return;
	...
	if (!prUserNewTitle)
		return;
	prUserNewTitle->SetColor(c_Color.r, c_Color.g, c_Color.b, c_Color.a);
	//TraceError("UpdateCustomTitleColor: r : %d  -- g %d  -- b %d",c_Color.r, c_Color.g, c_Color.b);
}

void CPythonTextTail::UpdateNewTitleValue(DWORD dwVID, const char* c_szText)
{
	TTextTailMap::iterator itor = m_CharacterTextTailMap.find(dwVID);
	if (m_CharacterTextTailMap.end() == itor)
		return;
	...
	TTextTail* pTextTail = itor->second;
	//TraceError("UpdateCustomTitleValue: %s",c_szText);
	CGraphicTextInstance*& prUserNewTitle = pTextTail->pUserNewTitleTextInstance;
	if (!prUserNewTitle)
		return;
...
void CPythonTextTail::AttachUserNewAlign(DWORD dwVID, const char* c_szText, const D3DXCOLOR& c_rColor)
{
	TTextTailMap::iterator itor = m_CharacterTextTailMap.find(dwVID);
	if (m_CharacterTextTailMap.end() == itor)
		return;
	...
	CGraphicTextInstance*& prUserNewAlign = pTextTail->pUserNewAlignTextInstance;
	if (!prUserNewAlign)
	{
		prUserNewAlign = CGraphicTextInstance::New();
		prUserNewAlign->SetTextPointer(ms_pFont);
		prUserNewAlign->SetOutline(true);

		prUserNewAlign->SetHorizonalAlign(CGraphicTextInstance::HORIZONTAL_ALIGN_RIGHT);
		prUserNewAlign->SetVerticalAlign(CGraphicTextInstance::VERTICAL_ALIGN_BOTTOM);
	}

	prUserNewAlign->SetValue(c_szText);
	prUserNewAlign->SetColor(c_rColor.r, c_rColor.g, c_rColor.b);
	prUserNewAlign->Update();
}

void CPythonTextTail::DetachUserNewAlign(DWORD dwVID)
{
	if (!bPKTitleEnable)
		return;
...
void CPythonTextTail::UpdateNewAlignColor(DWORD dwVID, const D3DXCOLOR& c_rColor)
{
	TTextTailMap::iterator itor = m_CharacterTextTailMap.find(dwVID);
	if (m_CharacterTextTailMap.end() == itor)
		return;
	...
	prUserNewAlign->SetColor(c_rColor.r, c_rColor.g, c_rColor.b, c_rColor.a);
}
void CPythonTextTail::UpdateNewAlignValue(DWORD dwVID, const char* c_szText)
{
	TTextTailMap::iterator itor = m_CharacterTextTailMap.find(dwVID);
	if (m_CharacterTextTailMap.end() == itor)
		return;
