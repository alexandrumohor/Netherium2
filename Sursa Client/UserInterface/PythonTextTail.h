...
class CPythonTextTail : public CSingleton<CPythonTextTail>
{
	public:
		typedef struct STextTail
		{
			...
			CGraphicTextInstance* pUserNewTitleTextInstance;
			CGraphicTextInstance* pUserNewAlignTextInstance;
#endif
...
#ifdef ENABLE_NEW_TITLE_SYSTEM
		void AttachUserNewAlign(DWORD dwVID, const char* c_szText, const D3DXCOLOR& c_rColor);
		void DetachUserNewAlign(DWORD dwVID);
		void UpdateNewAlignColor(DWORD dwVID, const D3DXCOLOR& c_rColor);
		void UpdateNewAlignValue(DWORD dwVID, const char* c_szText);

		void AttachUserNewTitle(DWORD dwVID, const char* c_szText, const D3DXCOLOR& c_rColor);
		void DetachUserNewTitle(DWORD dwVID);
		void UpdateNewTitleColor(DWORD dwVID, const D3DXCOLOR& c_rColor);
		void UpdateNewTitleValue(DWORD dwVID, const char* c_szText);
#endif
