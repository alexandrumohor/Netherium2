...
bool CPythonNetworkStream::RecvCharacterAppendPacket()
{
	...
	kNetActorData.m_stNewTitle = "";
	memset(kNetActorData.m_bNewTitleColor, 0, sizeof(kNetActorData.m_bNewTitleColor));

	kNetActorData.m_stNewAlign = "";
	memset(kNetActorData.m_bNewAlignColor, 0, sizeof(kNetActorData.m_bNewAlignColor));
#endif	
...
#ifdef ENABLE_NEW_TITLE_SYSTEM
		kNetActorData.m_stNewTitle = chrInfoPacket.newTitle.szName;
		memcpy(kNetActorData.m_bNewTitleColor, chrInfoPacket.newTitle.bCustomColours, sizeof(kNetActorData.m_bNewTitleColor));

		kNetActorData.m_stNewAlign = chrInfoPacket.newAlign.szName;
		memcpy(kNetActorData.m_bNewAlignColor, chrInfoPacket.newAlign.bColours, sizeof(kNetActorData.m_bNewAlignColor));
#endif
...
#ifdef ENABLE_NEW_TITLE_SYSTEM
bool CPythonNetworkStream::RecvCharacterUpdateNewTitlePacket()
{
	TPacketGCUpdateNewTitle chrUpdatePacket;
	if (!Recv(sizeof(chrUpdatePacket), &chrUpdatePacket))
		return false;

	m_rokNetActorMgr->UpdateActorNewTitle(chrUpdatePacket.dwVID, chrUpdatePacket.newTitle.szName, chrUpdatePacket.newTitle.bCustomColours);
	return true;
}

bool CPythonNetworkStream::RecvCharacterUpdateNewAlignPacket()
{
	TPacketGCUpdateNewAlign chrUpdatePacket;
	if (!Recv(sizeof(chrUpdatePacket), &chrUpdatePacket))
		return false;

	m_rokNetActorMgr->UpdateActorNewAlign(chrUpdatePacket.dwVID, chrUpdatePacket.newAlign.szName, chrUpdatePacket.newAlign.bColours);
	return true;
}
#endif