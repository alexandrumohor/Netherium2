...
class CInstanceBase
{
	public:
		struct SCreateData
		{
			...
			BYTE					m_bNewTitleColor[4];

			std::string				m_stNewAlign;
			BYTE					m_bNewAlignColor[4];
#endif
			BYTE	m_byPKMode;
...
#ifdef ENABLE_NEW_TITLE_SYSTEM
	public:
		void	UpdateNewTitleName(const char* szNewTitle);
		void	UpdateNewTitleColor(BYTE* customcolors);
		void	UpdateNewTitle(const char* szNewTitle, BYTE* customcolors);

		void	UpdateNewAlignName(const char* szNewAlign);
		void	UpdateNewAlignColor(BYTE* colors);
		void	UpdateNewAlign(const char* szNewAlign, BYTE* colors);
#endif
