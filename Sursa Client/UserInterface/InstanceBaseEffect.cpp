...
void CInstanceBase::AttachTextTail()
{
	...
	CPythonTextTail::instance().AttachUserNewTitle(GetVirtualID(), m_stNewTitle.c_str(), D3DXCOLOR(FLOAT(m_bNewTitleColor[0]) / 255.0, FLOAT(m_bNewTitleColor[1]) / 255.0, FLOAT(m_bNewTitleColor[2]) / 255.0, FLOAT(m_bNewTitleColor[3]) / 255.0));
	CPythonTextTail::instance().AttachUserNewAlign(GetVirtualID(), m_stNewAlign.c_str(), D3DXCOLOR(FLOAT(m_bNewAlignColor[0]) / 255.0, FLOAT(m_bNewAlignColor[1]) / 255.0, FLOAT(m_bNewAlignColor[2]) / 255.0, FLOAT(m_bNewAlignColor[3]) / 255.0));
#endif
}
...
#ifdef ENABLE_NEW_TITLE_SYSTEM
void CInstanceBase::UpdateNewTitleName(const char* szNewTitle)
{
	std::string stNewTitle = szNewTitle;

	if (stNewTitle == m_stNewTitle)
		return;

	m_stNewTitle = szNewTitle;
	CPythonTextTail::instance().UpdateNewTitleValue(GetVirtualID(), m_stNewTitle.c_str());
}

void CInstanceBase::UpdateNewTitleColor(BYTE* customcolors)
{
	if (memcmp(m_bNewTitleColor, customcolors, sizeof(BYTE) * NEW_COLOR_MAX_NUM) == 0)
		return;

	memcpy(m_bNewTitleColor, customcolors, sizeof(m_bNewTitleColor));
	CPythonTextTail::instance().UpdateNewTitleColor(GetVirtualID(), D3DXCOLOR(FLOAT(m_bNewTitleColor[0]) / 255.0, FLOAT(m_bNewTitleColor[1]) / 255.0, FLOAT(m_bNewTitleColor[2]) / 255.0, FLOAT(m_bNewTitleColor[3]) / 255.0));
}

void CInstanceBase::UpdateNewTitle(const char* szNewTitle, BYTE* customcolors)
{
	UpdateNewTitleName(szNewTitle);
	UpdateNewTitleColor(customcolors);
}


void CInstanceBase::UpdateNewAlignName(const char* szNewAlign)
{
	std::string stNewAlign = szNewAlign;

	if (stNewAlign == m_stNewAlign)
		return;

	m_stNewAlign = szNewAlign;
	CPythonTextTail::instance().UpdateNewAlignValue(GetVirtualID(), m_stNewAlign.c_str());
}

void CInstanceBase::UpdateNewAlignColor(BYTE* colors)
{
	if (memcmp(m_bNewAlignColor, colors, sizeof(BYTE) * NEW_COLOR_MAX_NUM) == 0)
		return;

	memcpy(m_bNewAlignColor, colors, sizeof(m_bNewAlignColor));
	CPythonTextTail::instance().UpdateNewAlignColor(GetVirtualID(), D3DXCOLOR(FLOAT(m_bNewAlignColor[0]) / 255.0, FLOAT(m_bNewAlignColor[1]) / 255.0, FLOAT(m_bNewAlignColor[2]) / 255.0, FLOAT(m_bNewAlignColor[3]) / 255.0));
}

void CInstanceBase::UpdateNewAlign(const char* szNewAlign, BYTE* colors)
{
	if (szNewAlign == " ") {
		CPythonTextTail::instance().DetachUserNewAlign(GetVirtualID());
	}
	else {
		UpdateNewAlignName(szNewAlign);
		UpdateNewAlignColor(colors);
	}
}
#endif